...
Migrations.add({
	...
	up() {
		...
		if (settingRetention) {
			Settings.upsert({
				_id: 'RetentionPolicy_DoNotExcludeTask',
			}, {
				$set: {
		...
		if (settingSidebar) {
			Settings.upsert({
				_id: 'Accounts_Default_User_Preferences_sidebarShowTask',
			}, {
				$set: {
		...
		Permissions.find({
			...
			const newId = perm._id === 'start-thread' ? 'start-task' : 'start-task-other-user';
